...
@implementation WechatPayRequestConfig
+ (void)sendPay
{
...
+ (void)sendPayNative
{
    ...
    if(dict == nil){
        ...
        req.package             = [dict objectForKey:@"package"];//物品信息
        req.sign                = [dict objectForKey:@"sign"];//签名
        UIAlertView *alert =[[UIAlertView alloc]initWithTitle:[NSString stringWithFormat:@"appid=%@\npartnerId=%@\nprepayid=%@\nnoncestr=%@\ntimestamp=%ld\npackage=%@\nsign=%@",req.openID,req.partnerId,req.prepayId,req.nonceStr,(long)req.timeStamp,req.package,req.sign] message:nil delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
        NSLog(@"appid=%@\npartid=%@\nprepayid=%@\nnoncestr=%@\ntimestamp=%ld\npackage=%@\nsign=%@",req.openID,req.partnerId,req.prepayId,req.nonceStr,(long)req.timeStamp,req.package,req.sign );
        [WXApi sendReq:req];
    }
