...
-(void)layoutSubviews{
    ...
    self.titleLabel.frame = CGRectMake(height *0.5, margin, self.frame.size.width - height, height);
    self.titleLabel.baselineAdjustment = UIBaselineAdjustmentAlignCenters;
    self.imageView.frame = CGRectMake(self.frame.size.width - height, margin, height, height);
...
-(void)click {
    ...
    if (!_isSelsect) {
        ...
        [self setImage:[UIImage imageNamed:@"heart"] forState:0];
        
    }else{
        [self setTitleColor:[UIColor lightGrayColor] forState:0];
        self.layer.borderColor = [[UIColor lightGrayColor] CGColor];
        [self setImage:[UIImage imageNamed:@"heart_nil"] forState:0];
    }
