...
@interface XianShiDingDanFaVC ()<UITableViewDataSource,UITableViewDelegate,SFTaskTableViewCellDelegate>
{
    int _pageNo;
...
@implementation XianShiDingDanFaVC
...
-(void)loadData:(BOOL)isAdd
{
    ...
    [RequestManager GetHuozhuLimitTaskWithuserId:[UserManager getDefaultUser].userId pageNo:[NSString stringWithFormat:@"%d",_pageNo] pageSize:@"5" Success:^(NSArray *result) {
        [self endRefresh];
        if (_pageNo == 1) {
...
{    
    ...
    __weak XianShiDingDanFaVC *weakSelf = self;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MyFaDetailViewController *MatInformationVC = [[MyFaDetailViewController alloc]init];
    ShunFeng *model = _modelArray[indexPath.section];
    VC.model = model;
    MatInformationVC.model = model;
    ...
    if ([model.status isEqualToString:@"0"]||[model.status isEqualToString:@"1"]||[model.status isEqualToString:@"6"]||[model.status isEqualToString:@"8"] || [model.status isEqualToString:@"9"]) {
        ...
        [self.navigationController pushViewController:MatInformationVC animated:YES];
    }else
    {
...
-(void)CreatPopView:(NSString *)money withModel:(ShunFeng *)model
{
    ...
    [_payView setModel:[NSArray arrayWithObjects:@"专程送",[NSString stringWithFormat:@"%@ 元",yunfei],[NSString stringWithFormat:@"%@元(保额：%@元)",_insuerCost,_premium],[NSString stringWithFormat:@"%@",_transferMoney],distance,nil]];
        ...
        switch (tag) {
            case 0:
            {
                if (!weakPayView.couponSwitch.isOn) {
                    _couponId = @"";
                    [weakPayView setModel:[NSArray arrayWithObjects:@"专程送",[NSString stringWithFormat:@"%@元",yunfei],[NSString stringWithFormat:@"%@元(保额：%@元)",_insuerCost,_premium],[NSString stringWithFormat:@"%@",_transferMoney],distance,nil]];
                    _couponArray = nil;
...
//            }
            ...
            NSLog(@"支付宝支付");
            [AlipayRequestConfig alipayWithPartner:kPartnerID seller:kSellerAccount tradeNO:model.billCode productName:@"镖王" productDescription: @"专程送" amount:moneyAliPay notifyURL:kNotifyURL itBPay:@"30m" standbyCallback:^(NSDictionary *resultDic)
             {
