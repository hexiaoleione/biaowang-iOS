...
@interface WuLiuBiaoViewController ()<UITableViewDelegate,UITableViewDataSource>{
...
@implementation WuLiuBiaoViewController
...
-(void)initUI{
    ...
    [shaixuanBtn setTitle:@"筛选目的地>>" forState:UIControlStateNormal];
    [shaixuanBtn addTarget:self action:@selector(shaixuan:) forControlEvents:UIControlEventTouchUpInside];
    [shaixuanBtn setTitleColor:[UIColor grayColor]forState:UIControlStateNormal];
    [self.view addSubview:shaixuanBtn];
    [self.view addSubview:self.tableView];

    _bgv = [[NothingBGView alloc] initWithFrame:CGRectMake(0, 40, WINDOW_WIDTH, WINDOW_HEIGHT)];
...
-(UITableView *)tableView{
    if (!_tableView) {
        _tableView = [[UITableView alloc]initWithFrame:CGRectMake(8, 40, WINDOW_WIDTH - 16, WINDOW_HEIGHT - 64-44-40) style:UITableViewStylePlain];
        ...
        __weak WuLiuBiaoViewController *weakSelf = self;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.distanceLabel.text = [NSString stringWithFormat:@"距离发货地:%@km",model.distance];
    cell.goodsName.text = model.cargoName;
    cell.destinationLabel.text = model.endPlaceName.length ? model.endPlaceName: model.entPlace;
    cell.weightLabel.text = model.cargoWeight;
    cell.squareLabel.text = [NSString stringWithFormat:@"%@方",model.cargoVolume];
    cell.quHuo.text = model.takeCargo?@"物流公司上门取货":@"发货人送到货场";
    cell.songHuo.text = model.sendCargo ? @"物流公司送货上门":@"收货人自提";
    ...
    cell.block = ^(id sennder){
        if ( [[UserManager getDefaultUser].wlid intValue] == 0) {
            [SVProgressHUD showInfoWithStatus:@"您还不是物流公司或大货司机,快去认证吧"];
            return;
        }else{
