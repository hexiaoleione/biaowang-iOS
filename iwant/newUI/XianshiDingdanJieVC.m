...
@interface XianshiDingdanJieVC ()<UITableViewDataSource,UITableViewDelegate>
{
    int _pageNo;
...
@implementation XianshiDingdanJieVC
...
-(void)loadData:(BOOL)isAdd
{
    ...
    _tableView.hidden = NO;
        [RequestManager GetBiaoShiLimitTaskRouteWithuserId:[UserManager getDefaultUser].userId pageNo:[NSString stringWithFormat:@"%d",_pageNo] pageSize:@"5" Success:^(NSArray *result) {
            [self endRefresh];
            if (_pageNo == 1) {
...
-(void)CreatTableView
{
    ...
    __weak XianshiDingdanJieVC *weakSelf = self;
