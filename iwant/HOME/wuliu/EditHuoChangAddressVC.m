...
@interface EditHuoChangAddressVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,BMKSuggestionSearchDelegate,UITextFieldDelegate,BMKPoiSearchDelegate,UIAlertViewDelegate>
{
...
@implementation EditHuoChangAddressVC
...
- (void)viewDidLoad {
    ...
    [self setCostomeTitle:@"修改货场地址"];
    self.view.backgroundColor = BACKGROUND_COLOR;
    self.localAdress.text =[[NSUserDefaults standardUserDefaults] valueForKey:USER_LOCASTR];
    self.searchTextFiled.text = self.model.locationAddress;
    self.detailAdressTextField.text = self.model.address;
    if (_model.ifDefault) {
...
- (IBAction)makeSureAdd:(UIButton *)sender {
    ...
    NSDictionary * dic = @{@"userId":[UserManager getDefaultUser].userId,
                           ...
                           @"cityCode":_cityCode?_cityCode:_model.cityCode,
                           @"longitude":_lo?_lo:_model.longitude,
                           @"latitude":_la?_la:_model.latitude,
                           @"ifDefault":self.switchDefault.on?@"1":@"0",
                           @"ifDelete":@"0"};
