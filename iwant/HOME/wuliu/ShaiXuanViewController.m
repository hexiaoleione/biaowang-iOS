...
@interface ShaiXuanViewController()<UITableViewDataSource,UITableViewDelegate>{
    ...
    NSString * _endPlaceCode ;//省市区的code
    NSString * _type;//后台为了区分省市区号
...
- (void)viewDidLoad {
    ...
    [_tableCity  mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo(self.view.frame.size.width/2);
        make.width.mas_equalTo(self.view.frame.size.width/2);
...
//    [self.view addSubview:self.tableCounty];
    ...
    UIBarButtonItem * rightItem =[[UIBarButtonItem alloc]initWithTitle:@"确认" style:UIBarButtonItemStyleDone target:self action:@selector(sure)];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == _tableProvince) {
        ...
        _endPlaceCode = province.provcode;
        _type =@"2";
        self.shaixuanBlock(_endPlaceCode,_type);
