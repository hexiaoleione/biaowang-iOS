...
- (void)setSelectIndex:(NSInteger)selectIndex animation:(BOOL)animation
{
    ...
    if (selectIndex < self.items.count) {
        ...
        self.lineLayer.frame = CGRectMake(CGRectGetMinX(item.frame)+ CGRectGetWidth(item.frame)/4, CGRectGetHeight(item.frame) - self.lineWidth, CGRectGetWidth(item.frame)/2, self.lineWidth);
