...
@interface XSFaViewConstroller ()<UITableViewDataSource,UITableViewDelegate>{
    ...
    UITextField * _otherPhonetextField;
...
@implementation XSFaViewConstroller
...
-(void)textFieldDidChange :(UITextField *)theTextField{
    ...
    if ([num floatValue] > 5000) {
        ...
        self.detailViewHeight.constant = 206*RATIO_HEIGHT;
    }else{
        self.matTypeBtn.hidden = YES;
        self.mateType.hidden = YES;
        self.mateType.text = @"";
...
-(void)publishSure{
    ...
    if (_temp.length !=0) {
       parametersDict = @{@"userId":[UserManager getDefaultUser].userId,
                                         ...
                                         @"limitTime":self.sfModel.limitTime,
    ...
    [ExpressRequest sendWithParameters:parametersDict MethodStr:API_PUBLISH_TASK reqType:k_POST success:^(id object) {
        [SVProgressHUD dismiss];
...
//        [self CreatPopView:_transferMoney];
        LCFPayViewConstroller * vc =[[LCFPayViewConstroller alloc]init];
        vc.payName = @"专程送";
        vc.model = _model;
        if ([self.sendType isEqualToString:@"1"]) {
            vc.sendType = self.sendType;
        }
        [self.navigationController pushViewController:vc animated:YES];
    } failed:^(NSString *error) {
        [SVProgressHUD showErrorWithStatus:error];
...
//            }
        ...
        case 3:
        {
            ...
            NSLog(@"支付宝支付");
            [AlipayRequestConfig alipayWithPartner:kPartnerID seller:kSellerAccount tradeNO:_sfModel.billCode productName:@"镖王" productDescription: @"专程送" amount:moneyAliPay notifyURL:kNotifyURL itBPay:@"30m" standbyCallback:^(NSDictionary *resultDic)
             {
