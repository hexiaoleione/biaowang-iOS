...
@interface JieSFViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>{
...
@implementation JieSFViewController
...
-(void)creatTableView{
    ...
    __weak JieSFViewController *weakSelf = self;
...
-(void)loadData:(BOOL)isAdd
{
    ...
    NSString *URLStr = [NSString stringWithFormat:@"%@%@?%@=%@&%@=%@&%@=%@&search=%@",BaseUrl,API_DOWNWIND_TASK_DRIVERFIND_DOWNWINDTASK,k_USER_ID,[UserManager getDefaultUser].userId,k_PAGE_NO,[NSString stringWithFormat:@"%d",_pageNo],k_PAGE_SIEZ,@"10",_search];
...
{
    JieSFCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ID"];
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"JieSFCell" owner:nil options:nil] lastObject];
                            ...
                            [self.navigationController pushViewController:webVC animated:YES];
                                    ...
                                    [self presentViewController:alert1 animated:YES completion:nil];
                                        ...
                                        if (model.ifReplaceMoney && model.replaceMoney.length >0 && ![model.replaceMoney isEqualToString:@""]) {
                                            messageStr = @"此订单为代收款订单，接单后将冻结您账户相应金额，待收款成功后，冻结金额会支付给发货人。请务必在用户要求时间内到达";
                                        }else{
...
-(void)confirmAcceptOrderModel:(ShunFengBiaoShi*)model {
