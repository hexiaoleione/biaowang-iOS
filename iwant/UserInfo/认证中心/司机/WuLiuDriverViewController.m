...
@interface WuLiuDriverViewController ()<YMHeaderViewDelegate>{
...
/*
 ...
 private String status;//个人用户状态  0->正在审核  1->审核通过 2->拒绝  3->禁用
...
@implementation WuLiuDriverViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.view.backgroundColor = BACKGROUND_COLOR;
    [self setCostomeTitle:@"大货司机认证"];
    [self creatUI];
...
-(void)headerView:(YMHeaderView *)headerView didSelectWithImage:(UIImage *)image{
    ...
    NSDictionary *dic = @{k_USER_ID:[UserManager getDefaultUser].userId,k_FILE_NAME:@"wuliuDriver.png"};
    NSDictionary *fileDic = @{@"data":image,@"fileName":@"wuliuDriver.png"};
