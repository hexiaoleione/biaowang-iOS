...
//            [self.view addSubview:_webView];
        ...
        case WEB_insureWL:{
            ...
            break;
        case WEB_insureSF:{
            urlStr = [NSString stringWithFormat:@"http://www.efamax.com/mobile/InsureSF.html?%@",strDate];
            title = @"顺风模块投保声明";
            NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:urlStr]];
            [_webView loadRequest:request];
            [_webView setScalesPageToFit:NO];
...
//    [request.URL.absoluteString hasPrefix:@"scht://?"];
    ...
    if (range.location != NSNotFound) {
        ...
        //E币不够兑换
        UIAlertView *alertV = [[UIAlertView alloc]initWithTitle:@"友情提示" message:@"您的E币不足，要多多积攒哦！" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
...
//        NSString *js = @"alert(\"没E币,你对换个大JB啊\");";
